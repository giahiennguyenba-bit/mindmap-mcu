...
void main()
{
    ...
    float baseFresnel = uFresnelOffset + (1.0 + dot(viewDirection, computedNormal)) * uFresnelMultiplier;
    
    // Sử dụng smoothstep thay cho max(0.0) để phần chuyển viền vào bóng râm được mềm mại
    float smoothedFresnel = smoothstep(0.0, 2.0, baseFresnel) * 2.0; 
    float fresnel = pow(smoothedFresnel, uFresnelPower);

    // Color
    // Nội suy cường độ sáng với smoothstep thay vì max(0.0) để tạo hiệu ứng tản sáng tự nhiên (Cinematic wrap lighting)
    float dotLightA = -dot(computedNormal.xyz, normalize(-uLightAPosition));
    float lightAIntensity = smoothstep(-0.2, 1.0, dotLightA) * uLightAIntensity;
    
    float dotLightB = -dot(computedNormal.xyz, normalize(-uLightBPosition));
    float lightBIntensity = smoothstep(-0.2, 1.0, dotLightB) * uLightBIntensity;

    vec3 color = vec3(0.0);
    color = mix(color, uLightAColor, lightAIntensity * fresnel);
    color = mix(color, uLightBColor, lightBIntensity * fresnel);
    
    // Highlight viền: Thay thế hàm clamp(pow(...)) gắt gỏng bằng smoothstep mượt mà, vệt sáng sẽ tan dần
    float highlight = smoothstep(0.8, 1.5, fresnel);
